...
class Database
{
	...
	{
		$this->connect();
		$result = $this->db->query("SELECT * FROM vanilla526, vanilla526_carousel, vanilla526_nav, vanilla526_seo");
	...
	{
		$this->connect();
		$result = $this->db->query("SELECT * FROM vanilla526, vanilla526_carousel, vanilla526_nav, vanilla526_seo WHERE ID = $id");
		$row = $result->fetch_assoc();
		if (is_null($row)) return null;

// Headline
function showHeadline() {
    var iframe = document.getElementById("frame1");
    var elmnt = iframe.contentWindow.document.getElementById("show-Headline");
    elmnt.style.position = "relative";
    elmnt.style.borderRadius = "25px";
    elmnt.style.boxShadow = "0 0 0 2000px rgba(0, 0, 0, 0.75)";
    elmnt.style.transition = "ease-in-out box-shadow 0.5s";
}

function delHeadline() {
    var iframe = document.getElementById("frame1");
    var elmnt = iframe.contentWindow.document.getElementById("show-Headline");
    elmnt.style.boxShadow = "none";
    elmnt.style.position = "static";
}

// StrongSubHeadline
function showStrongSubHeadline() {
    var iframe = document.getElementById("frame1");
    var elmnt = iframe.contentWindow.document.getElementById("show-StrongSubHeadline");
    elmnt.style.position = "relative";
    elmnt.style.borderRadius = "25px";
    elmnt.style.boxShadow = "0 0 0 2000px rgba(0, 0, 0, 0.75)";
    elmnt.style.transition = "ease-in-out box-shadow 0.5s";
}

function delStrongSubHeadline() {
    var iframe = document.getElementById("frame1");
    var elmnt = iframe.contentWindow.document.getElementById("show-StrongSubHeadline");
    elmnt.style.boxShadow = "none";
    elmnt.style.position = "static";
}

// HeaderCtaBtn
function showHeaderCtaBtn() {
    var iframe = document.getElementById("frame1");
    var elmnt = iframe.contentWindow.document.getElementById("show-HeaderCtaBtn");
    elmnt.style.position = "relative";
    elmnt.style.borderRadius = "25px";
    elmnt.style.boxShadow = "0 0 0 2000px rgba(0, 0, 0, 0.75)";
    elmnt.style.transition = "ease-in-out box-shadow 0.5s";
}

function delHeaderCtaBtn() {
    var iframe = document.getElementById("frame1");
    var elmnt = iframe.contentWindow.document.getElementById("show-HeaderCtaBtn");
    elmnt.style.boxShadow = "none";
    elmnt.style.position = "static";
}

// SubHeadLine
function showSubHeadline() {
    var iframe = document.getElementById("frame1");
    var elmnt = iframe.contentWindow.document.getElementById("show-SubHeadline");
    elmnt.style.position = "relative";
    elmnt.style.borderRadius = "25px";
    elmnt.style.boxShadow = "0 0 0 2000px rgba(0, 0, 0, 0.75)";
    elmnt.style.transition = "ease-in-out box-shadow 0.5s";
}

function delSubHeadline() {
    var iframe = document.getElementById("frame1");
    var elmnt = iframe.contentWindow.document.getElementById("show-SubHeadline");
    elmnt.style.boxShadow = "none";
    elmnt.style.position = "static";
}

// P2Header
function showP2Header() {
    var iframe = document.getElementById("frame1");
    var elmnt = iframe.contentWindow.document.getElementById("show-P2Header");
    elmnt.style.position = "relative";
    elmnt.style.borderRadius = "25px";
    elmnt.style.boxShadow = "0 0 0 2000px rgba(0, 0, 0, 0.75)";
    elmnt.style.transition = "ease-in-out box-shadow 0.5s";
}

function delP2Header() {
    var iframe = document.getElementById("frame1");
    var elmnt = iframe.contentWindow.document.getElementById("show-P2Header");
    elmnt.style.boxShadow = "none";
    elmnt.style.position = "static";
}

// P2HeaderText
function showP2HeaderText() {
    var iframe = document.getElementById("frame1");
    var elmnt = iframe.contentWindow.document.getElementById("show-P2HeaderText");
    elmnt.style.position = "relative";
    elmnt.style.borderRadius = "25px";
    elmnt.style.boxShadow = "0 0 0 2000px rgba(0, 0, 0, 0.75)";
    elmnt.style.transition = "ease-in-out box-shadow 0.5s";
}

function delP2HeaderText() {
    var iframe = document.getElementById("frame1");
    var elmnt = iframe.contentWindow.document.getElementById("show-P2HeaderText");
    elmnt.style.boxShadow = "none";
    elmnt.style.position = "static";
}

// P2HeaderListItem1
function showP2HeaderListItem1() {
    var iframe = document.getElementById("frame1");
    var elmnt = iframe.contentWindow.document.getElementById("show-P2HeaderListItem1");
    elmnt.style.position = "relative";
    elmnt.style.borderRadius = "25px";
    elmnt.style.boxShadow = "0 0 0 2000px rgba(0, 0, 0, 0.75)";
    elmnt.style.transition = "ease-in-out box-shadow 0.5s";
}

function delP2HeaderListItem1() {
    var iframe = document.getElementById("frame1");
    var elmnt = iframe.contentWindow.document.getElementById("show-P2HeaderListItem1");
    elmnt.style.boxShadow = "none";
    elmnt.style.position = "static";
}

// P2HeaderListItem2
function showP2HeaderListItem2() {
    var iframe = document.getElementById("frame1");
    var elmnt = iframe.contentWindow.document.getElementById("show-P2HeaderListItem2");
    elmnt.style.position = "relative";
    elmnt.style.borderRadius = "25px";
    elmnt.style.boxShadow = "0 0 0 2000px rgba(0, 0, 0, 0.75)";
    elmnt.style.transition = "ease-in-out box-shadow 0.5s";
}

function delP2HeaderListItem2() {
    var iframe = document.getElementById("frame1");
    var elmnt = iframe.contentWindow.document.getElementById("show-P2HeaderListItem2");
    elmnt.style.boxShadow = "none";
    elmnt.style.position = "static";
}

// P2HeaderListItem3
function showP2HeaderListItem3() {
    var iframe = document.getElementById("frame1");
    var elmnt = iframe.contentWindow.document.getElementById("show-P2HeaderListItem3");
    elmnt.style.position = "relative";
    elmnt.style.borderRadius = "25px";
    elmnt.style.boxShadow = "0 0 0 2000px rgba(0, 0, 0, 0.75)";
    elmnt.style.transition = "ease-in-out box-shadow 0.5s";
}

function delP2HeaderListItem3() {
    var iframe = document.getElementById("frame1");
    var elmnt = iframe.contentWindow.document.getElementById("show-P2HeaderListItem3");
    elmnt.style.boxShadow = "none";
    elmnt.style.position = "static";
}

// P2HeaderListItem4
function showP2HeaderListItem4() {
    var iframe = document.getElementById("frame1");
    var elmnt = iframe.contentWindow.document.getElementById("show-P2HeaderListItem4");
    elmnt.style.position = "relative";
    elmnt.style.borderRadius = "25px";
    elmnt.style.boxShadow = "0 0 0 2000px rgba(0, 0, 0, 0.75)";
    elmnt.style.transition = "ease-in-out box-shadow 0.5s";
}

function delP2HeaderListItem4() {
    var iframe = document.getElementById("frame1");
    var elmnt = iframe.contentWindow.document.getElementById("show-P2HeaderListItem4");
    elmnt.style.boxShadow = "none";
    elmnt.style.position = "static";
}

// P2HeaderCtaBtn
function showP2HeaderCtaBtn() {
    var iframe = document.getElementById("frame1");
    var elmnt = iframe.contentWindow.document.getElementById("show-P2HeaderCtaBtn");
    elmnt.style.position = "relative";
    elmnt.style.borderRadius = "25px";
    elmnt.style.boxShadow = "0 0 0 2000px rgba(0, 0, 0, 0.75)";
    elmnt.style.transition = "ease-in-out box-shadow 0.5s";
}

function delP2HeaderCtaBtn() {
    var iframe = document.getElementById("frame1");
    var elmnt = iframe.contentWindow.document.getElementById("show-P2HeaderCtaBtn");
    elmnt.style.boxShadow = "none";
    elmnt.style.position = "static";
}


//
// window.onload = function() {
//     let myiFrame = document.getElementById("myiFrame");
//     let doc = myiFrame.contentDocument;
//     doc.body.innerHTML = doc.body.innerHTML + '<style></style>';
// }